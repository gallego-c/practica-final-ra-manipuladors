...
\begin{enumerate}[leftmargin=2em, label=\textbf{\arabic*.}]
  ...
  \item \textbf{Proporcionar una interfície d'escaneig} basada en web per capturar i interpretar l'estat inicial del cub físic a través d'una càmera USB, amb calibratge interactiu de colors i previsualització 3D de la solució.
...
\begin{table}[H]
  ...
  \begin{tabular}{lp{9cm}}
    ...
    \texttt{change\_pick}& El robot fa un reagafat girant la pinça 90° sense deixar caure el cub. \\
...
\imgplaceholder[12cm]{Robot agafant el cub en holding-X (esquerra) i holding-Y (dreta)}{Dues orientacions d'agafada de la pinça OnRobot RG2 sobre el cub: \textit{holding-X} (eix X) i \textit{holding-Y} (eix Y).}
\label{fig:holding_xy}

\imgplaceholder[12cm]{Robot executant tilt\_x (esquerra) i tilt\_y (dreta)}{Accions de inclinació del cub: \texttt{tilt\_x} gira la cara esquerra a la part superior; \texttt{tilt\_y} gira la cara frontal a la part superior.}
\label{fig:tilt_xy}
...
\begin{tikzpicture}[
  ...
  \node[block=gray!15]  (scan)  {%
    \begin{tabular}{c}
      ...
      {\scriptsize Càmera USB · Interfície web · Calibratge de colors}
    \end{tabular}
  };
...
El procés comença amb l'\textbf{escaneig} del cub físic: l'usuari situa el cub davant d'una càmera USB i la interfície web captura i interpreta el color de cada adhesiu, generant la tupla d'estat inicial de 24 enters que descriu la configuració del cub.
...
\paragraph{Accions d'execució de moviments Rubik:}

Hi ha 18 accions \texttt{execute\_*}, una per a cada moviment estàndard (U, U', U2, D, D', D2, R, \ldots, B2). Totes comparteixen la mateixa estructura: requereixen que el robot tingui el cub agafat i que la \textbf{cara correcta estigui a la part superior}, i marquen el pas actual com a completat.

Un aspecte clau del disseny és que cada parella de cares oposades (U/D, R/L, F/B) \textbf{comparteix la mateixa acció física de gir de canell}. Per exemple, \texttt{execute\_U} pot executar tant un moviment U (quan la cara U és a dalt) com un moviment D (quan la cara D és a dalt, que físicament s'executa com un gir U vist des de l'altra banda). En el cas dels moviments D, L i B l'acció també actualitza els predicats d'orientació horitzontal de les cares laterals, ja que el gir de canell les reordena.
...
Mouen el robot a la configuració d'agafada sobre el fixture (IDLE\_X o IDLE\_Y) amb un \texttt{movej} i, un cop arribats, tanquen la pinça a 10\,mm via el script \texttt{gripper/pinza10UR3.py}. La diferència entre els dos és únicament la configuració articular de destí: \texttt{pick\_y} té el joint 6 girat 90° respecte \texttt{pick\_x}.

\subsubsection{\texttt{place.py}}

Obre la pinça a 40\,mm i mou el robot a la posició HOME via \texttt{go\_home.py}. S'executa sempre al principi i al final del pla per deixar el cub al fixture en estat conegut.

\subsubsection{\texttt{tilt\_x\_ur3.py} i \texttt{tilt\_y\_ur3.py}}

Executen la trajectòria de inclinació seguint una llista de \textit{waypoints} articulars (en radians) generada a partir de la trajectòria planificada per Kautham. Cada waypoint s'envia com a \texttt{movej} amb \texttt{blend=0.002\,m} per garantir fluïdesa. Un cop el robot arriba a la posició final (cub sobre el fixture en la nova orientació), s'obre la pinça a 40\,mm.
...
Les condicions d'il·luminació afecten directament la classificació de colors. Per adaptar-se a l'entorn, la pàgina \texttt{calibrate.html} permet capturar mostres de cada color del cub real i desar-ne els centroides RGB a \texttt{color\_calib.json}.
...
\imgplaceholder[13cm]{Captura de la interfície web d'escaneig mostrant el previsualitzador de càmera, la graella 2×2 i el mapa 2D del cub}{Interfície web d'escaneig: previsualitzador de càmera en directe, graella de captura i mapa 2D per revisar i corregir els colors detectats.}

% ═══════════════════════════════════════════════════════════════════════════════
%  (Continua amb la resta de seccions...)
% ═══════════════════════════════════════════════════════════════════════════════
